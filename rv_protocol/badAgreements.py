...
@aspectlib.Aspect
def _rotate_signed_pre_key_aspect(*args, **kwargs):
    state_current = args[0]
    force = False 
    if kwargs:
        force = kwargs["force"]
    spk = state_current._BaseState__signed_pre_key
    keyAge = state_current.signed_pre_key_age()
    rotation_period = state_current._State__signed_pre_key_rotation_period
    add_event("BEFORE", "rotate_signed_pre_key", watch = {
        "rotation_expected": (keyAge > rotation_period) or force 
    })
    try:
        yield
    finally:
